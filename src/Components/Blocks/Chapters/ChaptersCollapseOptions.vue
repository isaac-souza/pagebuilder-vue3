<template>
    <div class="">
        ...
        <div class="mt-8 mb-2 font-bold uppercase text-sm text-gray-500">Current chapters</div>
                    ...
                    <div class="overflow-hidden rounded-xl">
                        <table class="min-w-full divide-y divide-gray-200">
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="chapter in block.data.list" :key="chapter.uuid">
                                    <td class="px-2 py-1 whitespace-nowrap">
                                        {{ truncateText(chapter.title) }}
                                    </td>
                                    <td class="px-2 py-1 flex justify-end items-center my-auto py-auto">
                                        <button @click="remove(chapter.uuid)" class="btn btn-sm btn-ghost text-red-500">
    ...
    export default defineComponent({
        ...
        setup(props, context) {
            ...
            const remove = (uuidToRemove) => {
                const index = block.value.data.list.findIndex((chapter) => {
                    return chapter.uuid == uuidToRemove
                })
